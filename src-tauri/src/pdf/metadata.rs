...
pub fn update_metadata(
    ...
    if let Ok(Object::Dictionary(ref mut info)) = doc.get_object_mut(info_id) {
        if let Some(t) = title { info.set("Title", Object::String(t.into_bytes(), StringFormat::Literal)); }
...
pub fn batch_update_metadata(
    ...
    for path in paths {
        ...
        update_metadata(&path, title.clone(), author.clone(), subject.clone(), keywords.clone(), &output_path)?;
    }
    Ok(())
}
...
mod tests {
    ...
    fn test_update_metadata_success() {
        ...
        let info_id = output_doc.trailer.get(b"Info").unwrap().as_reference().unwrap();
