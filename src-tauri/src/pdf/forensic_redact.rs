...
#[tauri::command]
pub fn forensic_redact(path: &str, page_num: u32, rect: [f32; 4], output_path: &str) -> Result<(), String> {
    ...
    if let Some(page_id) = doc.get_pages().get(&page_num) {
        ...
        let content_ops = Content::decode(&content_bytes).map_err(|e| format!("Failed to decode redaction: {}", e))?;
