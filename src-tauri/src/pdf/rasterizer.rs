...
pub fn pdf_to_images(path: &str, output_dir: &str, format: &str) -> Result<Vec<String>, String> {
    ...
    let pdfium = Pdfium::new(
        Pdfium::bind_to_system_library()
            .map_err(|e| format!("Failed to bind to Pdfium system library: {:?}. Please ensure Pdfium is installed.", e))?,
    );
    ...
    for (index, page) in document.pages().iter().enumerate() {
        ...
        bitmap.as_image().save_with_format(&output_path_str, format_enum)
