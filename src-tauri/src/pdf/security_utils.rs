...
pub fn flatten_annotations(path: &str, output_path: &str) -> Result<(), String> {
    ...
    for (_page_num, page_id) in page_ids {
        ...
        for (annot_id, annot_dict) in annots_to_flatten {
            let rect = annot_dict.get(b"Rect").and_then(|obj| obj.as_array()).map_err(|_| "Invalid Rect")?;
            ...
            if let Ok(ap_dict) = annot_dict.get(b"AP").and_then(|obj| obj.as_dict()) {
                if let Ok(n_ref) = ap_dict.get(b"N").and_then(|obj| obj.as_reference()) {
                    ...
                    if let Ok(page_obj) = doc.get_object_mut(page_id) {
                        if let Ok(page_dict) = page_obj.as_dict_mut() {
                            ...
                            let xobjects = if let Ok(xobjs) = resources.get_mut(b"XObject") {
                                ...
                                resources.get_mut(b"XObject").unwrap().as_dict_mut().unwrap()
                            };
                    ...
                    let draw_command = format!("\nq\n1 0 0 1 {} {} cm\n/{} Do\nQ\n", x, y, xobj_name);
                    let content = Content::decode(&draw_command.into_bytes()).map_err(|e| e.to_string())?;
                    doc.add_to_page_content(page_id, content).map_err(|e: lopdf::Error| e.to_string())?;
                }
            }
        }
    ...
    doc.save(output_path).map_err(|e| format!("Failed to save flattened PDF: {}", e))?;
    Ok(())
}
